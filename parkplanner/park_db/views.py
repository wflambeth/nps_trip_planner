...
def events(request, park_code, date):
    ...
    endpoint = NPS_ENDPOINT + "/events?parkcode=" + park_code + "&dateStart=" + \
        date + "&api_key=" + API_KEY_NPS + "&expandRecurring=True"
    try:
...
def alerts(request, park_code):
    endpoint = NPS_ENDPOINT + "/alerts?parkcode=" + \
        park_code + "&api_key=" + API_KEY_NPS
    try:
        req = urllib.request.Request(endpoint, headers=HEADERS)
        with urllib.request.urlopen(req) as response:
